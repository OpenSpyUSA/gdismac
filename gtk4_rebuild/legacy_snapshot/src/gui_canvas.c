...
/* configure event */
/*******************/
static void canvas_update_geometry(GtkWidget *w, gint width, gint height)
{
gint size;
...
if (sysenv.width == width && sysenv.height == height && sysenv.size == size)
  return;
...
{
canvas_update_geometry(w, w->allocation.width, w->allocation.height);
...
/* widget realize hook */
/**********************/
static void canvas_realize(GtkWidget *w, gpointer data)
{
canvas_update_geometry(w, w->allocation.width, w->allocation.height);
redraw_canvas(ALL);
}
...
/****************************/
static void canvas_size_allocate(GtkWidget *w, GtkAllocation *allocation, gpointer data)
{
canvas_update_geometry(w, allocation->width, allocation->height);
}
...
gl_ready = gtk_widget_set_gl_capability(sysenv.glarea, sysenv.glconfig, NULL, TRUE, GDK_GL_RGBA_TYPE);
if (!gl_ready)
  g_warning("Failed to attach GL capability to the main canvas.");
...
g_signal_connect(GTK_OBJECT(sysenv.glarea), "size_allocate",
                 GTK_SIGNAL_FUNC(canvas_size_allocate), NULL);

	/* TODO - what about the "realize" event??? */
	g_signal_connect(GTK_OBJECT(sysenv.glarea), "realize",
	                 GTK_SIGNAL_FUNC(canvas_realize), NULL);
...
gtk_widget_add_events(GTK_WIDGET(sysenv.glarea), GDK_EXPOSURE_MASK
