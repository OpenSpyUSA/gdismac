...
gpointer gl_font;
gint gl_fontsize=10;
static gint gl_draw_offset_x=0;
static gint gl_draw_offset_y=0;
static gint gl_drawable_height=0;
static gdouble gl_draw_scale_x=1.0;
static gdouble gl_draw_scale_y=1.0;

gdouble gl_get_radius(struct core_pak *core, struct model_pak *model);

#ifdef __APPLE__
#define PIXEL_TOLERANCE 7
#define BOND_PIXEL_TOLERANCE 10
#else
#define PIXEL_TOLERANCE 5
#define BOND_PIXEL_TOLERANCE 8
#endif

/********************************************/
/* sync GL draw offset against Quartz layout */
/********************************************/
static void gl_sync_draw_offset(GdkGLDrawable *gldrawable)
{
gint drawable_width=0;
gdouble raw_scale_x=1.0, raw_scale_y=1.0;
#ifdef __APPLE__
gint paint_offset_x=0, paint_offset_y=0;
#endif

gl_draw_offset_x = 0;
gl_draw_offset_y = 0;
gl_drawable_height = sysenv.height;
gl_draw_scale_x = 1.0;
gl_draw_scale_y = 1.0;
...
gdk_gl_drawable_get_size(gldrawable, &drawable_width, &gl_drawable_height);

if (sysenv.width > 0 && sysenv.height > 0)
  {
  raw_scale_x = (gdouble) drawable_width / (gdouble) sysenv.width;
  raw_scale_y = (gdouble) gl_drawable_height / (gdouble) sysenv.height;

  /* Only enable scale correction when both axes agree on a real backing
   * scale change (eg. HiDPI), not when window chrome skews one dimension. */
  if (fabs(raw_scale_x - raw_scale_y) < 0.15 &&
      (fabs(raw_scale_x - 1.0) > 0.15 || fabs(raw_scale_y - 1.0) > 0.15))
    {
    gl_draw_scale_x = raw_scale_x;
    gl_draw_scale_y = raw_scale_y;
    }
  }

#ifdef __APPLE__
if (sysenv.glarea && sysenv.glarea->window)
  {
  GdkDrawable *paint_drawable=NULL;

  gdk_window_get_internal_paint_info(sysenv.glarea->window, &paint_drawable,
                                     &paint_offset_x, &paint_offset_y);

  if (paint_offset_x > 0)
    gl_draw_offset_x = paint_offset_x;
  if (paint_offset_y > 0)
    gl_draw_offset_y = paint_offset_y;
  }

if (sysenv.glarea && sysenv.glarea->window &&
    window && gtk_widget_get_realized(window) && window->window &&
    sysenv.width > 0 && sysenv.height > 0 &&
    (drawable_width > sysenv.width || gl_drawable_height > sysenv.height))
  {
  gint win_x=0, win_y=0, gl_x=0, gl_y=0;

  gdk_window_get_origin(window->window, &win_x, &win_y);
  gdk_window_get_origin(sysenv.glarea->window, &gl_x, &gl_y);

  if (gl_x - win_x > gl_draw_offset_x)
    gl_draw_offset_x = gl_x - win_x;
  if (gl_drawable_height - (gl_y - win_y) - sysenv.height > gl_draw_offset_y)
    gl_draw_offset_y = gl_drawable_height - (gl_y - win_y) - sysenv.height;

  if (gl_draw_offset_x < 0)
    gl_draw_offset_x = 0;
  if (gl_draw_offset_y < 0)
    gl_draw_offset_y = 0;
  }
#endif

if (g_getenv("GDIS_DEBUG_GL_OFFSET"))
  {
  fprintf(stderr,
          "gl offset: drawable=%dx%d canvas=%dx%d offset=%d,%d scale=%.3f,%.3f"
#ifdef __APPLE__
          " paint=%d,%d"
#endif
          "\n",
          drawable_width, gl_drawable_height,
          sysenv.width, sysenv.height,
          gl_draw_offset_x, gl_draw_offset_y,
          gl_draw_scale_x, gl_draw_scale_y
#ifdef __APPLE__
          , paint_offset_x, paint_offset_y
#endif
          );
...
/***********************************************/
/* convert world coords to widget-local coords */
/***********************************************/
static gboolean gl_project_window_coords(gdouble *px, gdouble *py, gdouble *pz,
                                         gdouble *w, struct canvas_pak *canvas)
{
GLdouble r[3];
...
gluProject(w[0], w[1], w[2],
           canvas->modelview, canvas->projection, canvas->viewport,
           &r[0], &r[1], &r[2]);

if (px)
  *px = (r[0] - gl_draw_offset_x) / gl_draw_scale_x;
if (py)
  /* Convert from whole-drawable GL coordinates back into the visible
   * widget's top-based coordinate space. */
  *py = (gl_draw_offset_y + sysenv.height * gl_draw_scale_y - r[1] - 1.0)
        / gl_draw_scale_y;
if (pz)
  *pz = r[2];
...
/**************************************/
/* can the given core be picked/drawn? */
/**************************************/
static gboolean gl_core_pickable(struct core_pak *core, struct model_pak *model)
{
if (!core || !model)
  return(FALSE);
...
/*******************************************/
/* camera axes for projected-radius picking */
/*******************************************/
static gboolean gl_pick_axes(gdouble *right, gdouble *up, struct model_pak *model)
{
struct camera_pak *camera;

if (!right || !up || !model || !model->camera)
  return(FALSE);

camera = model->camera;
ARR3SET(right, camera->e);
ARR3SET(up, camera->o);

if (camera->mode == LOCKED)
  {
  quat_rotate(right, camera->q);
  quat_rotate(up, camera->q);
  }
...
/*****************************************/
/* squared picking radius for screen hit */
/*****************************************/
static gdouble gl_core_pick_radius2(struct core_pak *core, struct canvas_pak *canvas,
                                    gdouble cx, gdouble cy)
{
gdouble radius, right[3], up[3], test[3], offset[3];
gdouble px, py, dx, dy, r2, min2;
...
min2 = PIXEL_TOLERANCE * PIXEL_TOLERANCE;
if (!gl_pick_axes(right, up, model))
  return(min2);

r2 = 0.0;

ARR3SET(test, core->rx);
ARR3SET(offset, right);
VEC3MUL(offset, radius);
ARR3ADD(test, offset);
gl_project_window_coords(&px, &py, NULL, test, canvas);
dx = px - cx;
dy = py - cy;
r2 = dx*dx + dy*dy;
...
/*************************************/
static gdouble gl_segment_distance2(gdouble px, gdouble py,
                                    gdouble x1, gdouble y1,
                                    gdouble x2, gdouble y2)
{
gdouble vx, vy, wx, wy, denom, t, dx, dy;
...
/*******************************************/
/* optional one-shot projection diagnostics */
/*******************************************/
static void gl_debug_dump_projected_cores(struct canvas_pak *canvas,
                                          struct model_pak *model)
{
...
if (!g_getenv("GDIS_DEBUG_PROJECTED_CORES"))
  return;
if (!canvas || !model)
  return;
if (dump_count >= 30)
...
fprintf(stderr, "projected cores[%d] for model: %s offset=%d,%d canvas=%dx%d\n",
        dump_count + 1,
        model->basename ? model->basename : "(unnamed)",
        gl_draw_offset_x, gl_draw_offset_y,
        sysenv.width, sysenv.height);
...
for (list=model->cores ; list ; list=g_slist_next(list))
  {
  core = list->data;
  if (!gl_core_pickable(core, model))
    continue;

  if (!gl_project_window_coords(&px, &py, &pz, core->rx, canvas))
    continue;

  fprintf(stderr, "  %2d %-8s -> x=%7.2f y=%7.2f z=%0.6f\n",
          count + 1,
          core->atom_label ? core->atom_label : "(atom)",
          px, py, pz);
...
#ifdef __APPLE__
sysenv.glconfig = gdk_gl_config_new_by_mode(mode);
sysenv.stereo_windowed = FALSE;
sysenv.render.stereo_quadbuffer = FALSE;
#else
sysenv.glconfig = gdk_gl_config_new_by_mode(mode | GDK_GL_STEREO);
if (sysenv.glconfig)
  {
...
glViewport(gl_draw_offset_x + canvas->x, gl_draw_offset_y + canvas->y,
...
px = gl_draw_offset_x + x * gl_draw_scale_x;
/* Map widget-local top-based Y into the visible GL viewport, not the full
 * drawable, otherwise clicks drift vertically when Quartz adds a top inset. */
py = gl_draw_offset_y + sysenv.height * gl_draw_scale_y
     - y * gl_draw_scale_y - 1.0;
...
gl_project_window_coords(&px, &py, NULL, w, canvas);
x[0] = nearest_int(px);
x[1] = nearest_int(py);
}
...
gl_project_window_coords(&px, &py, NULL, core->rx, canvas);
radius2 = gl_core_pick_radius2(core, canvas, px, py);
if (radius2 <= 0.0)
  return(G_MAXINT);
...
/* prefer the front-most visible hit and use pixel distance as a tie-breaker */
rmin = G_MAXINT;
zmin = G_MAXDOUBLE;
...
for (list=model->cores ; list ; list=g_slist_next(list))
  ...
  gl_project_window_coords(NULL, NULL, &depth, core->rx, canvas);
  if (depth < zmin - 1.0e-6 ||
      (fabs(depth - zmin) < 1.0e-6 && dr < rmin))
    {
    found = core;
    rmin = dr;
    zmin = depth;
...
gdouble d2, tol, tol2, depth, zmin;
gpointer match=NULL;
GSList *list;
...
tol = 0.5 * sysenv.render.stick_thickness + 3.0;
if (tol < BOND_PIXEL_TOLERANCE)
  tol = BOND_PIXEL_TOLERANCE;
tol2 = tol * tol;
zmin = G_MAXDOUBLE;
...
for (list=model->bonds ; list ; list=g_slist_next(list))
  ...
  if (!gl_core_pickable(core1, model) || !gl_core_pickable(core2, model))
    continue;

  gl_project_window_coords(&x1, &y1, &z1, core1->rx, canvas);
  gl_project_window_coords(&x2, &y2, &z2, core2->rx, canvas);

  d2 = gl_segment_distance2(x, y, x1, y1, x2, y2);
  if (d2 > tol2)
    continue;

  depth = (z1 < z2) ? z1 : z2;
  if (depth < zmin - 1.0e-6 ||
      (fabs(depth - zmin) < 1.0e-6 && d2 < tol2))
    {
    tol2 = d2;
    zmin = depth;
    match = bdata;
...
if (!sysenv.glarea || !gtk_widget_get_realized(sysenv.glarea) || !sysenv.glarea->window)
  return(FALSE);
...
printf("canvas: %p\nactive: %d\nresize: %d\nmodel: %p\n",
  ...
  gl_debug_dump_projected_cores(canvas, model);
