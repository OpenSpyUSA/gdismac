...
/* main window */
GtkWidget *window;
static GtkWidget *mac_actions_window = NULL;
static gint mac_viewer_target_width = 0;
static gint mac_viewer_target_height = 0;
static gboolean mac_viewer_window_sized = FALSE;
/* backing pixmap */
...
/*****************************/
/* debug widget geometry dump */
/*****************************/
static void gui_debug_widget_dump(const gchar *label, GtkWidget *widget)
{
gint wx=0, wy=0, ww=0, wh=0;

if (!widget)
  {
  fprintf(stderr, "layout[%s]: (null)\n", label);
  return;
  }
...
if (widget->window)
  ...
  gdk_window_get_geometry(widget->window, NULL, NULL, &ww, &wh, NULL);
  }

fprintf(stderr,
        "layout[%s]: visible=%d realized=%d mapped=%d alloc=%d,%d %dx%d req=%dx%d native=%d,%d %dx%d\n",
        label,
...
static gboolean gui_debug_layout_dump(gpointer data)
{
GtkWidget *top = data;
GtkWidget *menu_bar, *toolbar, *hpaned, *vpaned;

if (!g_getenv("GDIS_DEBUG_LAYOUT"))
  return(FALSE);
if (!GTK_IS_WIDGET(top))
  return(FALSE);

menu_bar = g_object_get_data(G_OBJECT(top), "gdis-menu-bar");
toolbar = g_object_get_data(G_OBJECT(top), "gdis-toolbar");
hpaned = g_object_get_data(G_OBJECT(top), "gdis-hpaned");
vpaned = g_object_get_data(G_OBJECT(top), "gdis-vpaned");

fprintf(stderr, "---- gtk layout dump ----\n");
gui_debug_widget_dump("window", top);
gui_debug_widget_dump("menu_bar", menu_bar);
gui_debug_widget_dump("toolbar", toolbar);
gui_debug_widget_dump("hpaned", hpaned);
gui_debug_widget_dump("vpaned", vpaned);
gui_debug_widget_dump("mpane", sysenv.mpane);
gui_debug_widget_dump("display_box", sysenv.display_box);
gui_debug_widget_dump("glarea", sysenv.glarea);
gui_debug_widget_dump("tpane", sysenv.tpane);
fprintf(stderr, "-------------------------\n");
...
/* event info */
x = (gint) floor(event->x + 0.5);
y = (gint) floor(event->y + 0.5);
sysenv.moving = FALSE;
...
if (g_getenv("GDIS_DEBUG_PICK"))
  {
  FILE *fp = fopen("/tmp/gdis-pick.log", "a");
  if (fp)
    {
    fprintf(fp, "pick press at %d,%d -> core=%s\n",
            x, y, core ? core->atom_label : "(none)");
    fclose(fp);
    }
  }
...
#define DEFINE_VIEW_WRAPPER(name)                               \
static void cb_view_##name(GtkWidget *widget, gpointer data)    \
{                                                               \
  (void) widget;                                                \
  (void) data;                                                  \
  name();                                                       \
...
DEFINE_VIEW_WRAPPER(gui_view_x)
DEFINE_VIEW_WRAPPER(gui_view_y)
DEFINE_VIEW_WRAPPER(gui_view_z)
DEFINE_VIEW_WRAPPER(gui_view_a)
DEFINE_VIEW_WRAPPER(gui_view_b)
DEFINE_VIEW_WRAPPER(gui_view_c)
...
gui_button(" x ", cb_view_gui_view_x, NULL, hbox, TT);
gui_button(" y ", cb_view_gui_view_y, NULL, hbox, TT);
gui_button(" z ", cb_view_gui_view_z, NULL, hbox, TT);
gui_button(" a ", cb_view_gui_view_a, NULL, hbox, TT);
gui_button(" b ", cb_view_gui_view_b, NULL, hbox, TT);
gui_button(" c ", cb_view_gui_view_c, NULL, hbox, TT);
}
...
static gboolean cb_window_gdis_exit_event(GtkWidget *widget,
                                          GdkEvent *event,
                                          gpointer data)
{
  (void) widget;
  (void) event;
  (void) data;
  return(gdis_exit_event());
}

static void cb_window_gdis_exit(GtkWidget *widget, gpointer data)
{
  (void) widget;
  (void) data;
  gdis_exit();
...
/*****************************************************/
/* restore the main left/right split after first map */
/*****************************************************/
gboolean cb_main_pane_init(gpointer data)
{
GtkWidget *pane = data;
gint width, position;
...
#ifdef __APPLE__
if (window && g_object_get_data(G_OBJECT(window), "gdis-left-pane-host"))
  {
...
position = sysenv.tree_width;
if (position < 160)
  position = width / 3;
if (position > width - 160)
  position = width / 2;
...
/********************************************/
/* show the GL widget once pane layout wins */
/********************************************/
gboolean cb_canvas_init(gpointer data)
{
#ifdef __APPLE__
if (sysenv.glarea)
  {
  ...
  if (sysenv.display_box)
    gtk_widget_queue_resize(sysenv.display_box);
...
/*****************************************************/
/* restore the display/text split after first layout */
/*****************************************************/
gboolean cb_display_pane_init(gpointer data)
{
GtkWidget *pane = data;
gint height, tray, position;
...
tray = sysenv.tray_height;
if (tray < 60)
  tray = 60;
if (tray > height - 120)
  tray = height / 3;

position = height - tray;
if (position < 120)
  position = height / 2;

#ifdef __APPLE__
if (sysenv.tpane)
  gtk_widget_hide(sysenv.tpane);
gtk_paned_set_position(GTK_PANED(pane), height);
g_timeout_add(100, cb_canvas_init, NULL);
return(FALSE);
#endif
...
/*************************************/
/* safe toolbar callback adaptors    */
/* GTK2 expects clicked handlers to  */
/* accept (GtkWidget *, gpointer).   */
/*************************************/
#define DEFINE_TOOLBAR_VOID_WRAPPER(name)                         \
static void cb_toolbar_##name(GtkWidget *widget, gpointer data)   \
{                                                                 \
  (void) widget;                                                  \
  (void) data;                                                    \
  name();                                                         \
...
DEFINE_TOOLBAR_VOID_WRAPPER(file_load_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(file_save_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(edit_model_create)
DEFINE_TOOLBAR_VOID_WRAPPER(tree_select_delete)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_edit_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_render_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_gperiodic_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(select_all)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_measure_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_isosurf_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_diffract_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(surface_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_plots_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_dock_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_mdi_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_zmat_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gulp_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_siesta_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_vasp_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_uspex_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(task_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_setup_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_about_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(space_image_widget_reset)
DEFINE_TOOLBAR_VOID_WRAPPER(canvas_single)
DEFINE_TOOLBAR_VOID_WRAPPER(canvas_create)
DEFINE_TOOLBAR_VOID_WRAPPER(canvas_delete)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_view_default)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_view_x)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_view_y)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_view_z)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_view_a)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_view_b)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_view_c)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_rotate_x)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_rotate_y)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_rotate_z)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_animate_dialog)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_graph_controls)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_eps_export)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_track_output)
DEFINE_TOOLBAR_VOID_WRAPPER(gui_help_dialog)

static void cb_toolbar_mode_switch(GtkWidget *widget, gpointer data)
{
gtk_mode_switch(widget, GPOINTER_TO_INT(data));
}

#ifdef __APPLE__
static GtkWidget *gui_macos_action_button(const gchar *label,
                                          gpointer callback,
                                          GtkWidget *box)
{
GdkColor button_bg = {0, 0xd800, 0xd800, 0xd800};
GdkColor button_bg_active = {0, 0xc400, 0xc400, 0xc400};
GdkColor text_fg = {0, 0x0000, 0x0000, 0x0000};
GtkWidget *button, *child;

button = gui_button((gchar *) label, callback, NULL, box, 0);
gtk_widget_set_size_request(button, -1, 28);
gtk_widget_modify_bg(button, GTK_STATE_NORMAL, &button_bg);
gtk_widget_modify_bg(button, GTK_STATE_PRELIGHT, &button_bg_active);
gtk_widget_modify_bg(button, GTK_STATE_ACTIVE, &button_bg_active);

child = gtk_bin_get_child(GTK_BIN(button));
if (child)
  {
  gtk_widget_modify_fg(child, GTK_STATE_NORMAL, &text_fg);
  gtk_widget_modify_fg(child, GTK_STATE_PRELIGHT, &text_fg);
  gtk_widget_modify_fg(child, GTK_STATE_ACTIVE, &text_fg);
  }

return(button);
}

static gboolean cb_mac_actions_keep_visible(GtkWidget *widget,
                                            GdkEvent *event,
                                            gpointer data)
{
(void) widget;
...
static void gui_macos_viewer_window_arrange(void)
{
GdkScreen *screen;
gint screen_width, viewer_width, viewer_height, viewer_x;
...
if (mac_viewer_target_width < 200)
  mac_viewer_target_width = (sysenv.width > 0) ? sysenv.width : 880;
if (mac_viewer_target_height < 200)
  mac_viewer_target_height = (sysenv.height > 0) ? sysenv.height : 640;

if (!mac_viewer_window_sized)
  {
  gtk_window_resize(GTK_WINDOW(window),
                    mac_viewer_target_width,
                    mac_viewer_target_height);
  mac_viewer_window_sized = TRUE;
  }

gtk_window_get_size(GTK_WINDOW(window), &viewer_width, &viewer_height);
if (viewer_width < 200)
  viewer_width = mac_viewer_target_width;
if (viewer_height < 200)
  viewer_height = mac_viewer_target_height;

viewer_x = screen_width - viewer_width - 28;
if (viewer_x < 472)
  viewer_x = 472;

gtk_window_move(GTK_WINDOW(window), viewer_x, 78);
gtk_window_present(GTK_WINDOW(window));
}

static void gui_macos_controls_window_arrange(void)
{
GdkScreen *screen;
gint screen_width, viewer_width, controls_width, viewer_x, controls_x;

if (!mac_actions_window)
  return;

screen = gtk_window_get_screen(GTK_WINDOW(mac_actions_window));
if (!screen)
  screen = gdk_screen_get_default();
if (!screen)
  return;
...
screen_width = gdk_screen_get_width(screen);
gtk_window_get_size(GTK_WINDOW(window), &viewer_width, NULL);
if (viewer_width < 200)
  viewer_width = (mac_viewer_target_width > 0) ? mac_viewer_target_width : 880;
controls_width = 420;
viewer_x = screen_width - viewer_width - 28;
if (viewer_x < 472)
  viewer_x = 472;

controls_x = viewer_x - controls_width - 24;
if (controls_x < 24)
  controls_x = 24;

gtk_window_move(GTK_WINDOW(mac_actions_window), controls_x, 78);
}

static void gui_macos_actions_palette_present(void)
{
GtkWidget *vbox, *label, *row;
GtkWidget *menu_bar, *toolbar_host, *quick_actions_host, *left_pane_host;
GtkWidget *hpaned;
GdkColor text_fg = {0, 0x0000, 0x0000, 0x0000};
gint wx = 80, wy = 140;
...
if (mac_actions_window)
  {
  gui_macos_viewer_window_arrange();
  gui_macos_controls_window_arrange();
  gtk_window_present(GTK_WINDOW(mac_actions_window));
  return;
  }

menu_bar = g_object_get_data(G_OBJECT(window), "gdis-menu-bar");
toolbar_host = g_object_get_data(G_OBJECT(window), "gdis-toolbar-host");
quick_actions_host = g_object_get_data(G_OBJECT(window), "gdis-quick-actions-host");
left_pane_host = g_object_get_data(G_OBJECT(window), "gdis-left-pane-host");
hpaned = g_object_get_data(G_OBJECT(window), "gdis-hpaned");

if (menu_bar)
  gtk_widget_hide(menu_bar);
if (toolbar_host)
  gtk_widget_hide(toolbar_host);
if (quick_actions_host)
  gtk_widget_hide(quick_actions_host);
if (left_pane_host)
  {
  gtk_widget_set_size_request(left_pane_host, 1, -1);
  gtk_widget_hide(left_pane_host);
  }
if (GTK_IS_PANED(hpaned))
gtk_paned_set_position(GTK_PANED(hpaned), 1);

mac_actions_window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
gtk_window_set_title(GTK_WINDOW(mac_actions_window), "GDIS Controls");
gtk_window_set_default_size(GTK_WINDOW(mac_actions_window), 520, 900);
gtk_window_set_resizable(GTK_WINDOW(mac_actions_window), TRUE);
gtk_window_set_type_hint(GTK_WINDOW(mac_actions_window), GDK_WINDOW_TYPE_HINT_UTILITY);
gtk_window_set_keep_above(GTK_WINDOW(mac_actions_window), TRUE);
gtk_container_set_border_width(GTK_CONTAINER(mac_actions_window), PANEL_SPACING);
g_signal_connect(GTK_OBJECT(mac_actions_window), "delete-event",
                 GTK_SIGNAL_FUNC(cb_mac_actions_keep_visible), NULL);

vbox = gtk_vbox_new(FALSE, PANEL_SPACING);
gtk_container_add(GTK_CONTAINER(mac_actions_window), vbox);

if (menu_bar)
  {
  if (gtk_widget_get_parent(menu_bar))
    gtk_widget_reparent(menu_bar, vbox);
  else
    gtk_box_pack_start(GTK_BOX(vbox), menu_bar, FALSE, FALSE, 0);
  gtk_widget_show(menu_bar);
  }

label = gtk_label_new("Linux-style menus and common tools");
gtk_widget_modify_fg(label, GTK_STATE_NORMAL, &text_fg);
gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 0);

row = gtk_hbox_new(FALSE, PANEL_SPACING);
gtk_box_pack_start(GTK_BOX(vbox), row, FALSE, FALSE, 0);
gui_macos_action_button("Open", cb_toolbar_file_load_dialog, row);
gui_macos_action_button("Save", cb_toolbar_file_save_dialog, row);
gui_macos_action_button("New", cb_toolbar_edit_model_create, row);
gui_macos_action_button("Edit", cb_toolbar_gui_edit_dialog, row);

row = gtk_hbox_new(FALSE, PANEL_SPACING);
gtk_box_pack_start(GTK_BOX(vbox), row, FALSE, FALSE, 0);
gui_macos_action_button("Render", cb_toolbar_gui_render_dialog, row);
gui_macos_action_button("Measure", cb_toolbar_gui_measure_dialog, row);
gui_macos_action_button("Iso", cb_toolbar_gui_isosurf_dialog, row);
gui_macos_action_button("Surface", cb_toolbar_surface_dialog, row);

row = gtk_hbox_new(FALSE, PANEL_SPACING);
gtk_box_pack_start(GTK_BOX(vbox), row, FALSE, FALSE, 0);
gui_macos_action_button("Diffraction", cb_toolbar_gui_diffract_dialog, row);
gui_macos_action_button("Plots", cb_toolbar_gui_plots_dialog, row);
gui_macos_action_button("Animate", cb_toolbar_gui_animate_dialog, row);
gui_macos_action_button("Periodic", cb_toolbar_gui_gperiodic_dialog, row);

row = gtk_hbox_new(FALSE, PANEL_SPACING);
gtk_box_pack_start(GTK_BOX(vbox), row, FALSE, FALSE, 0);
gui_macos_action_button("Dock", cb_toolbar_gui_dock_dialog, row);
gui_macos_action_button("Dynamics", cb_toolbar_gui_mdi_dialog, row);
gui_macos_action_button("Zmatrix", cb_toolbar_gui_zmat_dialog, row);
gui_macos_action_button("Reset View", cb_toolbar_gui_view_default, row);

row = gtk_hbox_new(FALSE, PANEL_SPACING);
gtk_box_pack_start(GTK_BOX(vbox), row, FALSE, FALSE, 0);
gui_macos_action_button("GULP", cb_toolbar_gulp_dialog, row);
gui_macos_action_button("SIESTA", cb_toolbar_gui_siesta_dialog, row);
gui_macos_action_button("VASP", cb_toolbar_gui_vasp_dialog, row);
gui_macos_action_button("USPEX", cb_toolbar_gui_uspex_dialog, row);

row = gtk_hbox_new(FALSE, PANEL_SPACING);
gtk_box_pack_start(GTK_BOX(vbox), row, FALSE, FALSE, 0);
gui_macos_action_button("Tasks", cb_toolbar_task_dialog, row);
gui_macos_action_button("Setup", cb_toolbar_gui_setup_dialog, row);
gui_macos_action_button("About", cb_toolbar_gui_about_dialog, row);
gui_macos_action_button("Manual", cb_toolbar_gui_help_dialog, row);

label = gtk_label_new("Model browser and property panels");
gtk_widget_modify_fg(label, GTK_STATE_NORMAL, &text_fg);
gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 0);

if (sysenv.mpane && gtk_widget_get_parent(sysenv.mpane))
  gtk_widget_reparent(sysenv.mpane, vbox);
else if (sysenv.mpane)
  gtk_box_pack_start(GTK_BOX(vbox), sysenv.mpane, TRUE, TRUE, 0);

gui_macos_viewer_window_arrange();
gtk_widget_show_all(mac_actions_window);
gui_macos_controls_window_arrange();
gtk_window_present(GTK_WINDOW(mac_actions_window));
}

static gboolean cb_macos_actions_palette_present(gpointer data)
{
(void) data;

gui_macos_actions_palette_present();

return(FALSE);
}

void gui_macos_present_windows(void)
{
gui_macos_actions_palette_present();
...
GtkWidget *toolbar_host, *left_pane_host, *right_pane_host;
#ifdef __APPLE__
GtkWidget *display_host, *status_host, *quick_actions_host, *quick_actions;
#endif
GdkBitmap *mask;
...
#ifdef __APPLE__
sysenv.visual = gdk_visual_get_system();
sysenv.colourmap = gdk_colormap_get_system();
if (!sysenv.visual || !sysenv.colourmap)
  {
  printf("Error: could not get system visual/colourmap.\n");
  exit(1);
  }
...
if (!sysenv.visual)
  {
  printf("Error: could not get a usable visual.\n");
  exit(1);
  }
sysenv.depth = sysenv.visual->depth;
...
#ifdef __APPLE__
gtk_window_set_title(GTK_WINDOW(window), "GDIS Viewer");
#else
...
g_object_set_data(G_OBJECT(window), "gdis-menu-bar", menu_bar);
...
#ifdef __APPLE__
toolbar_host = gtk_event_box_new();
gtk_box_pack_start(GTK_BOX(vbox), toolbar_host, FALSE, FALSE, 0);
hbox = gtk_hbox_new(FALSE, 0);
gtk_container_add(GTK_CONTAINER(toolbar_host), hbox);
g_object_set_data(G_OBJECT(window), "gdis-toolbar-host", toolbar_host);
#else
hbox = gtk_hbox_new(FALSE, 0);
...
g_object_set_data(G_OBJECT(window), "gdis-toolbar", toolbar);
...
gtk_toolbar_append_item(GTK_TOOLBAR(toolbar), NULL,
                        ...
                        gdis_wid, GTK_SIGNAL_FUNC(cb_toolbar_file_load_dialog), NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar), NULL,
                        ...
                        GTK_SIGNAL_FUNC(cb_toolbar_file_save_dialog), NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_edit_model_create),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_tree_select_delete),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_edit_dialog),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_render_dialog),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_gperiodic_dialog),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_select_all),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_measure_dialog),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_isosurf_dialog),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_diffract_dialog),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_surface_dialog),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR(toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_space_image_widget_reset),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_canvas_single),
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_canvas_create),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_canvas_delete),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR(toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_view_default),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_view_x),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_view_y),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_view_z),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_view_a),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_view_b),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_view_c),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_rotate_x),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_rotate_y),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_rotate_z),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_gui_animate_dialog),
                        NULL);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
                        ...
                        "Private",
                        gdis_wid,
                        GTK_SIGNAL_FUNC(cb_toolbar_mode_switch),
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
			...
			"Private",
			gdis_wid,
			GTK_SIGNAL_FUNC(cb_toolbar_gui_graph_controls),
			NULL);
/* eps/png export tool */
if(sysenv.have_eps){
	...
	gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
			...
			"Private",
			gdis_wid,
			GTK_SIGNAL_FUNC(cb_toolbar_gui_eps_export),
			NULL);
}else{
	pixbuf = image_table_lookup("image_to_eps");
	gdis_wid = gtk_image_new_from_pixbuf(pixbuf);
...
gtk_toolbar_append_item(GTK_TOOLBAR (toolbar),
			...
			"Private",
			gdis_wid,
			GTK_SIGNAL_FUNC(cb_toolbar_gui_track_output),
			NULL);
#ifdef __APPLE__
quick_actions_host = gtk_event_box_new();
gtk_box_pack_start(GTK_BOX(vbox), quick_actions_host, FALSE, FALSE, 0);
quick_actions = gtk_hbox_new(FALSE, PANEL_SPACING);
gtk_widget_set_size_request(quick_actions_host, -1, 44);
gtk_widget_set_size_request(quick_actions, -1, 44);
gtk_container_set_border_width(GTK_CONTAINER(quick_actions), PANEL_SPACING);
gtk_container_add(GTK_CONTAINER(quick_actions_host), quick_actions);
g_object_set_data(G_OBJECT(window), "gdis-quick-actions-host", quick_actions_host);
{
GdkColor actions_bg = {0, 0xf000, 0xf000, 0xf000};

gtk_widget_modify_bg(quick_actions_host, GTK_STATE_NORMAL, &actions_bg);
gtk_widget_modify_bg(quick_actions, GTK_STATE_NORMAL, &actions_bg);
}

gui_macos_action_button("Open", cb_toolbar_file_load_dialog, quick_actions);
gui_macos_action_button("Save", cb_toolbar_file_save_dialog, quick_actions);
gui_macos_action_button("New", cb_toolbar_edit_model_create, quick_actions);
gui_macos_action_button("Edit", cb_toolbar_gui_edit_dialog, quick_actions);
gui_macos_action_button("Render", cb_toolbar_gui_render_dialog, quick_actions);
gui_macos_action_button("Measure", cb_toolbar_gui_measure_dialog, quick_actions);
gui_macos_action_button("Surface", cb_toolbar_surface_dialog, quick_actions);
gui_macos_action_button("Diffraction", cb_toolbar_gui_diffract_dialog, quick_actions);
#endif
/* MAIN LEFT/RIGHT HBOX PANE */
/* paned window */
hpaned = gtk_hpaned_new();
gtk_container_add(GTK_CONTAINER(GTK_BOX(vbox)), hpaned);
g_object_set_data(G_OBJECT(window), "gdis-hpaned", hpaned);

/* LEFT PANE */
#ifdef __APPLE__
left_pane_host = gtk_event_box_new();
gtk_paned_pack1(GTK_PANED(hpaned), left_pane_host, TRUE, TRUE);
sysenv.mpane = gtk_vbox_new(FALSE, 0);
gtk_container_add(GTK_CONTAINER(left_pane_host), sysenv.mpane);
gtk_widget_set_size_request(left_pane_host, sysenv.tree_width, -1);
g_object_set_data(G_OBJECT(window), "gdis-left-pane-host", left_pane_host);
#else
sysenv.mpane = gtk_vbox_new(FALSE, 0);
gtk_paned_pack1(GTK_PANED(hpaned), sysenv.mpane, TRUE, TRUE);
gtk_widget_set_size_request(sysenv.mpane, sysenv.tree_width, -1);
#endif
if (sysenv.tree_width > 0)
...
/* RIGHT PANE */
#ifdef __APPLE__
right_pane_host = gtk_event_box_new();
gtk_paned_pack2(GTK_PANED(hpaned), right_pane_host, TRUE, TRUE);
vbox = gtk_vbox_new(FALSE, 0);
gtk_container_add(GTK_CONTAINER(right_pane_host), vbox);
#else
vbox = gtk_vbox_new(FALSE, 0);
...
g_object_set_data(G_OBJECT(window), "gdis-vpaned", vpaned);

#ifdef __APPLE__
display_host = gtk_event_box_new();
gtk_paned_pack1(GTK_PANED(vpaned), display_host, TRUE, TRUE);
sysenv.display_box = gtk_vbox_new(FALSE, 0);
gtk_container_add(GTK_CONTAINER(display_host), sysenv.display_box);
#else
sysenv.display_box = gtk_vbox_new(FALSE, 0);
...
#ifdef __APPLE__
status_host = gtk_event_box_new();
gtk_container_add(GTK_CONTAINER(status_host), sysenv.tpane);
gtk_paned_pack2(GTK_PANED(vpaned), status_host, TRUE, TRUE);
#else
...
if (sysenv.tray_height > 0 && sysenv.height > sysenv.tray_height)
  gtk_paned_set_position(GTK_PANED(vpaned), sysenv.height);
...
g_signal_connect(GTK_OBJECT(window), "delete-event", GTK_SIGNAL_FUNC(cb_window_gdis_exit_event), NULL);
/* save gdisrc callback */
g_signal_connect(GTK_OBJECT(window), "destroy", GTK_SIGNAL_FUNC(cb_window_gdis_exit), NULL);
...
g_idle_add(cb_macos_actions_palette_present, NULL);
#endif
g_idle_add(gui_debug_layout_dump, window);
g_idle_add(cb_main_pane_init, hpaned);
g_idle_add(cb_display_pane_init, vpaned);
g_timeout_add(500, gui_debug_layout_dump, window);
