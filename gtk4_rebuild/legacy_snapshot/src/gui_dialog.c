...
/**************************************/
/* bring a dialog to the foreground */
/**************************************/
static void dialog_present_widget(GtkWidget *w, gpointer data)
{
if (GTK_IS_WINDOW(w))
  {
...
static void dialog_present(struct dialog_pak *dialog)
{
if (!dialog)
  return;
if (!GTK_IS_WIDGET(dialog->window))
  return;

gtk_widget_show(dialog->window);
dialog_present_widget(dialog->window, NULL);
}
...
if (window && GTK_IS_WINDOW(window) && window != dialog->window)
  {
  gtk_window_set_transient_for(GTK_WINDOW(dialog->window), GTK_WINDOW(window));
  gtk_window_set_destroy_with_parent(GTK_WINDOW(dialog->window), TRUE);
  gtk_window_set_position(GTK_WINDOW(dialog->window), GTK_WIN_POS_CENTER_ON_PARENT);
  }
g_signal_connect_after(GTK_OBJECT(dialog->window), "show",
                       GTK_SIGNAL_FUNC(dialog_present_widget), NULL);
...
if (!curr_path || !GTK_IS_LABEL(curr_path))
  return(FALSE);
if (!file_path_ts || !GTK_IS_LIST_STORE(file_path_ts))
  return(FALSE);
if (!file_name_ts || !GTK_IS_LIST_STORE(file_name_ts))
  return(FALSE);
...
if (!file_name || !GTK_IS_ENTRY(file_name))
  return;
...
/* selection handlers */
/**********************/
static void file_path_open_row(GtkTreeView *treeview, GtkTreePath *treepath)
{
gchar *text, *path;
GtkTreeIter iter;
...
static gboolean cb_file_path_cursor_changed_idle(gpointer data)
{
...
if (!file_path_tv || !GTK_IS_TREE_VIEW(file_path_tv))
  return(FALSE);

selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(file_path_tv));
if (!selection)
  return(FALSE);
...
file_path_open_row(GTK_TREE_VIEW(file_path_tv), treepath);
gtk_tree_path_free(treepath);

return(FALSE);
}

static void file_path_activate(GtkTreeView *treeview,
                               GtkTreePath *treepath,
                               ...
                               gpointer data)
{
  (void) column;
  (void) data;
  file_path_open_row(treeview, treepath);
}

static void cb_file_path_cursor_changed(GtkTreeView *treeview, gpointer data)
{
  (void) treeview;
  (void) data;
  g_idle_add(cb_file_path_cursor_changed_idle, NULL);
}
...
static void file_name_activate(GtkTreeView *treeview,
                               GtkTreePath *treepath,
                               GtkTreeViewColumn *column,
                               gpointer data)
{
...
/* cleanup on exit */
/*******************/
void file_cleanup(gpointer data)
{
/* unused */
(void) data;
...
if (file_path_ts)
  if (GTK_IS_LIST_STORE(file_path_ts))
    gtk_list_store_clear(file_path_ts);
if (file_name_ts)
  if (GTK_IS_LIST_STORE(file_name_ts))
    gtk_list_store_clear(file_name_ts);
free_slist(dir_list);
dir_list = NULL;
...
dialog = dialog_request(FILE_SELECT, "File dialog", NULL, file_cleanup, NULL);
if (!dialog)
  return;
...
select = gtk_tree_view_get_selection(GTK_TREE_VIEW(file_path_tv));
gtk_tree_selection_set_mode(select, GTK_SELECTION_BROWSE);
g_signal_connect(file_path_tv, "cursor-changed",
                 G_CALLBACK(cb_file_path_cursor_changed),
                 NULL);
